...
import { Plane, MapPin, PlaneTakeoff, RotateCcw, Clock, Activity, CheckCircle, XCircle, Hourglass, RefreshCcw,} from 'lucide-react';
...
const SearchFlight = ({ setTab }) => {
  ...
  useEffect(() => {
    ...
    const filtered = flights.filter((flight) => {
      ...
      const matchesSearch = searchType === 'flight' ? flightNumber.includes(search) : arrivalAirport.includes(search);

      const matchesStatus = statusFilter === 'All' || flight.flight_status?.toLowerCase() === statusFilter.toLowerCase();
      ...
      <div className="max-w-full mx-auto lg:w-3/4 px-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow mb-8">
          <div className="flex gap-2 mb-3">
            ...
            <button className={`flex items-center gap-2 px-4 py-2 rounded-full border transition text-sm ${searchType === 'flight' ? 'border-blue-500 text-blue-600 bg-blue-50 font-medium' : 'border-gray-300 text-gray-500 bg-gray-100'}`} onClick={() => setSearchType('flight')}>
              <Plane className="w-4 h-4" />
              Flight
            </button>

            <button className={`flex items-center gap-2 px-4 py-2 rounded-full border transition text-sm ${searchType === 'destination' ? 'border-blue-500 text-blue-600 bg-blue-50 font-medium' : 'border-gray-300 text-gray-500 bg-gray-100'}`}onClick={() => setSearchType('destination')}>
          ...
          <div className="flex items-center border border-gray-300 rounded-full overflow-hidden">
            ...
            <input type="text" className="flex-1 py-2 pr-3 text-sm focus:outline-none" placeholder={`Enter ${searchType === 'flight' ? 'flight number (e.g., AA123)' : 'arrival airport (e.g., JFK)'}`} value={inputValue}onChange={(e) => setInputValue(e.target.value)}/>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:max-w-xs bg-white rounded-2xl shadow-lg p-4 flex flex-col mb-4 h-90 overflow-auto">
              ...
              <div className="flex flex-col gap-2">
                {[
                  ...
                  <label key={status.value} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="status" value={status.value} checked={statusFilter === status.value} onChange={() => setStatusFilter(status.value)} className="accent-blue-500 w-3 h-3" />
            ...
            <button type="button" className="mt-4 w-full flex items-center justify-center gap-2 border border-gray-200 rounded-full py-1.5 bg-gray-50 text-gray-600 text-sm font-medium transition hover:bg-blue-50 hover:text-blue-600 hover:border-blue-300 active:scale-95" onClick={() => { setStatusFilter('All'); setInputValue(''); setSearchType('flight'); }} >
          ...
          <div className="flex-1 space-y-4">
            ...
            {filteredFlights.map((flight, idx) => {
              ...
              return (
                <div key={`${flight.flight?.iata}-${idx}`} className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col sm:flex-row items-start sm:items-stretch px-5 py-4 gap-6 cursor-pointer group border border-gray-100 hover:border-blue-200" >
                  ...
                  <div className="flex items-center sm:items-start justify-end min-w-[120px]">
                    ...
                    <span className={`inline-flex items-center px-4 py-1.5 rounded-full font-medium text-sm gap-2 ${badge.bg} ${badge.color} group-hover:scale-105 transition-transform duration-200`}>
                      {badge.icon}
